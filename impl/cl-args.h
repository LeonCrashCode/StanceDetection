...
struct Params {
  ...
  unsigned bilstm_input_dim = 100;
  unsigned bilstm_hidden_dim = 150;
  unsigned nonlinear_dim = 64;
...
void get_args(int argc,
  ...
  while (i < argc) {
    ...
    if (arg == "--name" || arg == "-n") {
      ...
      i++;
    } else if (arg == "--nonlinear_dim") {
      if (i + 1 == argc) {
        std::cerr << "No matching argument for " << arg << std::endl;
        abort();
      }
      istringstream d(argv[i + 1]);
      d >> params.nonlinear_dim;
      i++;
